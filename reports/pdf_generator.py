...
def _header_footer(canvas, doc):
    ...
    canvas.drawString(15 * mm, 9 * mm,
                      "Generated by Watershed Insight - automated geospatial evidence platform")
    canvas.drawRightString(width - 15 * mm, 9 * mm, f"Page {doc.page}")
    canvas.restoreState()


def _new_doc(path: str, title: str, author: str = "Watershed Insight"):
...
def generate_intervention_pdf(store, intervention_id: str, output_path: str,
    ...
    story.append(sign)
    story.append(Paragraph(
        f"Document generated {dt.datetime.now().strftime('%d %b %Y, %H:%M')} by Watershed Insight "
